...
function openssl_build(){
    ...
    if ! sudo wget -P '/opt' $openssl_tarrball &>/dev/null; then
        ...
        write_log 1 "OpenSSL ${openssl_version} Downloaded successfully"
    fi

    write_log 3 "Extracting OpenSSL files"
    #
    sudo mkdir $openssl_path  &>/dev/null
    #
    if ! sudo tar -xvf $openssl_tarball_path -C $openssl_path --strip-components=1 &>/dev/null; then
        ...
        write_log 1 "OpenSSL tarball successfully removed"
    fi

    write_log 3 "Building OpenSSL ${openssl_version}"
    sudo mkdir /opt/openssl &>/dev/null
